...
<body class="bg-gray-50 text-gray-800 font-sans antialiased flex flex-col min-h-screen">
    <nav
        class="w-full z-50 sticky top-0 left-0 bg-linear-to-r from-brand-dark to-brand-blue shadow-lg border-b border-blue-800">

        <div class="mx-auto px-6 lg:px-8 ">
            <div id="menu-backdrop"
                class="fixed top-20 left-0 w-full h-[calc(100vh-5rem)] bg-gray-900/60 z-30 opacity-0 pointer-events-none transition-opacity duration-300 ease-in-out">
            </div>
            <div id="side-menu"
                class="fixed top-20 left-0 w-72 bg-linear-to-r from-[#0e162a] to-[#11203f] h-[calc(100vh-5rem)] border-t border-white shadow-2xl z-40 transform -translate-x-full transition-transform duration-300 ease-in-out overflow-y-auto">
                <div class="px-4 py-6 space-y-1 flex flex-col h-full">


                    <div class="p-3 border-b border-gray-200 flex flex-col items-center text-center ">
                        <div
                            class="w-16 h-16 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-400 shadow-sm mb-2">
                            <i class="fa-solid fa-user text-2xl text-gray-800"></i>
                        </div>

                        <div>
                            @auth
                                <h3 class="text-md mt-1 font-bold text-white">{{ auth()->user()->name }}</h3>
                            @endauth
                        </div>
                    </div>

                    <a href="{{ route('home') }}"
                        class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
                    ...
                    @auth
                        ...
                        <a href="{{ route('complaints.log') }}"
                            class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
                            <i class="fa-solid fa-user-clock w-5 text-center"></i> Transaction History
                        </a>
                    @endauth
                    @if(auth()->check() && in_array(auth()->user()->role, ['worker', 'supervisor']))
                        <a href="{{ route('worker.assignments') }}"
                            class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
                            <i class="fa-solid fa-clipboard-check w-5 text-center"></i> My Assignments
                        </a>
                    @endif
                    @if(auth()->check() && auth()->user()->role === 'dispatcher')
                        <a href="{{ route('dispatcher.job_orders.index') }}"
                            class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
                    ...
                    @if(auth()->check() && auth()->user()->role === 'admin')
                        ...
                        <a href="{{ route('admin.job-title.index') }}"
                            class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
                            <i class="fa-solid fa-id-badge w-5 text-center"></i> Job Title Modifications
                        </a>
                    ...
                    @auth
                        <div class="mt-auto">
                            ...
                            <a href="{{ auth()->user()->employee ? route('employee.profile.show') : route('citizen.profile.show') }}"
                                class="px-4 py-3 rounded-md text-sm font-medium text-white hover:text-brand-orange hover:bg-white/5 transition flex items-center gap-3">
            ...
            <div class="flex items-center justify-between h-20">
                <div class="flex justify-between items-center gap-10">
                    @guest
                        <div class="flex items-center gap-4 sm:gap-6">
                            <a href="{{ route('home') }}"
                                class="text-white hover:text-brand-orange transition text-sm font-medium flex items-center gap-2">
                    ...
                    @auth
                        ...
                        <div class="relative">
                            <button type="button" id="notification-btn"
                                class="relative text-white hover:text-brand-orange transition focus:outline-none ml-2 mt-1">
                                <i class="fa-regular fa-bell text-xl cursor-pointer"></i>
                                @if(auth()->user()->unreadNotifications->count() > 0)
                                    <span class="absolute -top-1 -right-1 flex h-3 w-3">
                                        <span
                                            class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75 cursor-pointer pointer-events-none"></span>
                                        <span
                                            class="relative inline-flex rounded-full h-3 w-3 bg-red-500 border-2 border-white cursor-pointer"></span>
                                    </span>
                                @endif
                            </button>

                            <div id="notification-dropdown"
                                class="hidden absolute max-h-112 left-0 mt-4 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
                                <div
                                    class="bg-gray-50 border-b border-gray-100 px-4 py-3 flex justify-between items-center">
                                ...
                                <div class="max-h-80 overflow-y-auto">
                                    @forelse (auth()->user()->notifications()->take(10)->get() as $notification)
                                        <a href="{{ route('notifications.read', $notification->id) }}"
                                            ...
                                            <div>
                                                <p class="text-sm font-semibold text-gray-800 group-hover:text-brand-blue"><i
                                                        class="fa-solid fa-{{ $notification->data['icon'] }} text-brand-blue text-sm"></i>
                                ...
                                @if(auth()->user()->notifications()->count() > 0)
                                    <div class="bg-gray-50 border-t border-gray-100 px-4 py-2 text-center">
                                        <form action="{{ route('notifications.markAllRead') }}" method="get">
                                            <button class="text-xs text-brand-orange font-bold hover:underline pointer"
                        ...
                        @if (auth()->user()->employee)
                            <div class="flex items-center gap-3 border-l border-white/20 pl-4 ml-2 hidden sm:flex">
                                ...
                                <button type="button" id="duty-toggle-btn" data-url="{{ route('employee.toggle-duty') }}"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-blue-200 text-[10px] tracking-wider uppercase">Smart Complaint Management System
                        </p>
                    </div>
                </div>
                @guest
                    <div>
                        <form action="{{ route('logout') }}">
                            @csrf
                            <a href="#" id="open-auth-btn"
                                class="md:inline-block cursor-pointer bg-brand-orange hover:bg-orange-600 text-white px-6 py-2 rounded shadow-lg transition font-semibold text-sm">
                                Sign In
                            </a>
                        </form>
                    </div>
    ...
    <div class="fixed top-24 left-4 z-20 flex flex-col gap-3 w-full">
        @if(session('success'))
            <div class="max-w-8xl px-6 lg:px-8 mt-4 alert-wrapper">
                <div class="bg-green-100 border-l-4 border-green-500 w-full text-green-700 p-4 rounded shadow-sm flex justify-between items-center"
                    role="alert">
                    <p>{{ session('success') }}</p>
                    <button type="button" onclick="this.closest('.alert-wrapper').remove()"
                        class="close-alert-btn text-green-700 hover:text-green-900 cursor-pointer transition focus:outline-none p-1"
                        aria-label="Close">
        ...
        @if(session('warning') && is_null(auth()->user()->national_no))
            <div class="max-w-8xl px-6 lg:px-8 mt-4 alert-wrapper">
                <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded shadow-sm flex justify-between items-center"
                    role="alert">
                    <p>{{ session('warning') }}</p>
                    <button type="button" onclick="this.closest('.alert-wrapper').remove()"
                        class="close-alert-btn text-green-700 hover:text-green-900 cursor-pointer transition focus:outline-none p-1"
    ...
    <div id="auth-modal" data-auto-open="{{ $autoOpenView }}"
        class="fixed inset-0 z-100 hidden flex items-center justify-center bg-gray-900/90 transition-opacity">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto relative">
            <button id="close-auth-btn"
                class="absolute top-4 left-4 text-gray-400 hover:text-gray-600 focus:outline-none transition cursor-pointer">
            ...
            <div id="login-view" class="p-8">
                ...
                <div class="space-y-4">
                    ...
                    <a href="{{ route('google.redirect') }}"
                        class="pointer w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition-all duration-200 hover:border-slate-500 hover:shadow-lg">
                ...
                <form method="POST" action="{{ route('login.submit') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('login-email') }}" type="email" name="login-email"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                        @error('login-email')
                    ...
                    <button type="submit"
                        class="w-full bg-brand-blue hover:bg-blue-800 text-white font-bold py-3 rounded-lg shadow transition mt-2 cursor-pointer">
                        Sign In
                    </button>
                </form>
                <p class="mt-6 text-sm text-center text-gray-600">
                    ...
                    <button id="show-register-btn"
                        class="text-brand-orange font-bold hover:underline focus:outline-none cursor-pointer">
                        Sign up here
                    </button>
                </p>
            </div>
            ...
            <div id="register-view" class="p-8 hidden">
                ...
                <form action="{{ route('register') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ old('national-no') }}" name="national-no"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                        @error('national-no')
                            <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="email" value="{{ old('register-email') }}" name="register-email"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                    ...
                    <div>
                        ...
                        <input id="phone-1" type="tel" value="{{ old('phone_full') }}" name="phone_full"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
            ...
            <div id="oauth-complete-view" class="p-8 hidden">
                ...
                <p class="text-sm text-gray-600 mb-6 text-center">We just need a few official details to secure your
                    account.</p>
                <form action="{{ route('oauth.finish') }}" method="post" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Phone Number:</label>
                        <input id="phone-2" type="tel" value="{{ old('phone_full') }}" name="phone_full"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                        <input type="hidden" value="{{ old('country_code') }}" id="country_code" name="country_code">
                        @error('phone_full')
                            <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
